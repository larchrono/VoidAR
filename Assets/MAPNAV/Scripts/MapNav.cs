...
public class MapNav : MonoBehaviour 
{
	...
	IEnumerator MapPosition(){
	   ...
	   //------------------------------------------------------------------	//
		...
		string mapStyle = "&style=feature:poi|element:labels|visibility:off";
		multiplier=1;
		url= "http://maps.googleapis.com/maps/api/staticmap?center="+fixLat+","+fixLon+"&zoom="+zoom+"&scale=2&language=zh-TW&size=640x640&format="+mapFormat[indexFormat]+"&maptype="+maptype[indexType]+"&sensor=false" + mapStyle + "&key="+key;
		tempLat = fixLat;
		tempLon = fixLon;
		*/
		...
		key = "XGivCKgE8sNHJcEASW2hNjGAFStk6WrA";
